...
class BaseDRS(object):
    ...
    def turn_on(self):
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("turn_on"))
        return self._transfer(send_packet, 6)

    def turn_off(self):
        """Turns off power supply"""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("turn_off"))
        return self._transfer(send_packet, 6)

    def open_loop(self):
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("open_loop"))
        return self._transfer(send_packet, 6)

    def closed_loop(self):
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("closed_loop"))
        return self._transfer(send_packet, 6)

    def reset_interlocks(self):
        """Resets interlocks on connected DRS device"""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("reset_interlocks"))
        return self._transfer(send_packet, 6)
    ...
    def set_slowref(self, setpoint: float) -> bytes:
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_slowref")) + hex_setpoint
        return self._transfer(send_packet, 6)

    def set_slowref_fbp(self, iRef1: int = 0, iRef2: int = 0, iRef3: int = 0, iRef4: int = 0) -> bytes:
    ...
    def set_slowref_readback_mon(self, setpoint: float) -> bytes:
        ...
        hex_setpoint = float_to_hex(setpoint)
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_slowref_readback_mon")) + hex_setpoint
        )
        reply_msg = self._transfer(send_packet, 9)
        val = struct.unpack("BBHfB", reply_msg)
        return val[3]

    def set_slowref_fbp_readback_mon(self, iRef1: int = 0, iRef2: int = 0, iRef3: int = 0, iRef4: int = 0):
    ...
    def set_slowref_readback_ref(self, setpoint: float) -> float:
        ...
        hex_setpoint = float_to_hex(setpoint)
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_slowref_readback_ref")) + hex_setpoint
        )
        reply_msg = self._transfer(send_packet, 9)
        val = struct.unpack("BBHfB", reply_msg)
        return val[3]

    def set_slowref_fbp_readback_ref(self, iRef1: int = 0, iRef2: int = 0, iRef3: int = 0, iRef4: int = 0):
    ...
    def set_param(self, param_id: int, n: int, value: float) -> bytes:
        """Set parameter"""
        # TODO: Turn into property?
        payload_size = size_to_hex(1 + 2 + 2 + 4)  # Payload: ID + param id + [n] + value
        ...
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_param")) + hex_id + hex_n + hex_value
        )
    ...
    def get_param(self, param_id, n=0):
        ...
        if type(param_id) == int:
            hex_id = double_to_hex(param_id)
        hex_n = double_to_hex(n)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("get_param")) + hex_id + hex_n
        self._reset_input_buffer()
        try:
            reply_msg = self._transfer(send_packet, 9)
            val = struct.unpack("BBHfB", reply_msg)
    ...
    def save_param_eeprom(self, param_id: int, n: int = 0, type_memory: int = 2) -> bytes:
        """Save parameter to EEPROM"""
        # TODO: Raise exception instead of printing?
        payload_size = size_to_hex(1 + 2 + 2 + 2)  # Payload: ID + param id + [n] + memory type
        if type(param_id) == str:
            hex_id = double_to_hex(list_parameters.index(param_id))
        if type(param_id) == int:
            hex_id = double_to_hex(param_id)
        ...
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("save_param_eeprom")) + hex_id + hex_n + hex_type
        )
        return self._transfer(send_packet, 6)

    def load_param_eeprom(self, param_id: int, n: int = 0, type_memory: int = 2) -> bytes:
        ...
        hex_type = double_to_hex(type_memory)
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("load_param_eeprom")) + hex_id + hex_n + hex_type
        )
        reply_msg = self._transfer(send_packet, 6)
        return reply_msg
    ...
    def save_param_bank(self, type_memory: int = 2) -> bytes:
        ...
        hex_type = double_to_hex(type_memory)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("save_param_bank")) + hex_type
    ...
    def load_param_bank(self, type_memory: int = 2) -> bytes:
        ...
        payload_size = size_to_hex(1 + 2)  # Payload: ID + memory type
        hex_type = double_to_hex(type_memory)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("load_param_bank")) + hex_type
        return self._transfer(send_packet, 6)
    ...
    def save_dsp_coeffs_eeprom(self, dsp_class: int, dsp_id: int, type_memory: int = 2) -> bytes:
        payload_size = size_to_hex(1 + 2 + 2 + 2)
        hex_dsp_class = double_to_hex(dsp_class)
        hex_dsp_id = double_to_hex(dsp_id)
        hex_type = double_to_hex(type_memory)
    ...
    def load_dsp_coeffs_eeprom(self, dsp_class: int, dsp_id: int, type_memory: int = 2) -> bytes:
        payload_size = size_to_hex(1 + 2 + 2 + 2)
        hex_dsp_class = double_to_hex(dsp_class)
        hex_dsp_id = double_to_hex(dsp_id)
        hex_type = double_to_hex(type_memory)
    ...
    def save_dsp_modules_eeprom(self, type_memory: int = 2) -> bytes:
        payload_size = size_to_hex(1 + 2)  # Payload: ID + memory type
        hex_type = double_to_hex(type_memory)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("save_dsp_modules_eeprom")) + hex_type
        return self._transfer(send_packet, 6)

    def load_dsp_modules_eeprom(self, type_memory: int = 2) -> bytes:
        payload_size = size_to_hex(1 + 2)  # Payload: ID + memory type
        hex_type = double_to_hex(type_memory)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("load_dsp_modules_eeprom")) + hex_type
        return self._transfer(send_packet, 6)
    ...
    def reset_udc(self):
        ...
        if reply == "Y" or reply == "y":
            payload_size = size_to_hex(1)  # Payload: ID
            send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("reset_udc"))
    ...
    def cfg_source_scope(self, p_source: int) -> bytes:
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("cfg_source_scope")) + hex_op_mode
        return self._transfer(send_packet, 6)

    def cfg_freq_scope(self, freq: float) -> bytes:
        payload_size = size_to_hex(1 + 4)  # Payload: ID + freq
        hex_op_mode = float_to_hex(freq)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("cfg_freq_scope")) + hex_op_mode
        return self._transfer(send_packet, 6)
    ...
    def cfg_duration_scope(self, duration: float) -> bytes:
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("cfg_duration_scope")) + hex_op_mode
        return self._transfer(send_packet, 6)

    def enable_scope(self) -> bytes:
        """Enables scope"""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("enable_scope"))
        return self._transfer(send_packet, 6)

    def disable_scope(self) -> bytes:
        """Disables scope"""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("disable_scope"))
        return self._transfer(send_packet, 6)
    ...
    def sync_pulse(self) -> bytes:
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("sync_pulse"))
        return self._transfer(send_packet, 6)
    ...
    def select_op_mode(self, op_mode: int) -> bytes:
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("select_op_mode")) + hex_op_mode
        return self._transfer(send_packet, 6)

    def set_serial_termination(self, term_enable: int) -> bytes:
        payload_size = size_to_hex(1 + 2)  # Payload: ID + enable
        hex_enable = double_to_hex(term_enable)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_serial_termination")) + hex_enable
        return self._transfer(send_packet, 6)
    ...
    def set_command_interface(self, interface: int) -> bytes:
        ...
        send_packet = (
            COM_FUNCTION + payload_size + index_to_hex(list_func.index("set_command_interface")) + hex_interface
        )
        return self._transfer(send_packet, 6)
    ...
    def unlock_udc(self, password: int) -> bytes:
        ...
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("unlock_udc")) + hex_password
        return self._transfer(send_packet, 6)
    ...
    def lock_udc(self, password: int) -> bytes:
        ...
        payload_size = size_to_hex(1 + 2)  # Payload: ID + password
        hex_password = double_to_hex(password)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("lock_udc")) + hex_password
        return self._transfer(send_packet, 6)

    def reset_counters(self) -> bytes:
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("reset_counters"))
        return self._transfer(send_packet, 6)
    ...
    def enable_siggen(self) -> bytes:
        ...
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("enable_siggen"))
        return self._transfer(send_packet, 6)

    def disable_siggen(self) -> bytes:
        """Disables signal generator"""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("disable_siggen"))
        return self._transfer(send_packet, 6)
    ...
    def cfg_wfmref(
        ...
        payload_size = size_to_hex(1 + 2 + 2 + 4 + 4 + 4)  # Payload: ID + idx + sync_mode + frequency + gain + offset
    ...
    def select_wfmref(self, idx: int) -> bytes:
        ...
        hex_idx = double_to_hex(idx)
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("select_wfmref")) + hex_idx
        return self._transfer(send_packet, 6)
    ...
    def reset_wfmref(self) -> bytes:
        ...
        waveform's start."""
        payload_size = size_to_hex(1)  # Payload: ID
        send_packet = COM_FUNCTION + payload_size + index_to_hex(list_func.index("reset_wfmref"))
        return self._transfer(send_packet, 6)
    ...
    def Write_sigGen_Freq(self, float_value):
        ...
        send_packet = (
            COM_WRITE_VAR + WRITE_FLOAT_SIZE_PAYLOAD + index_to_hex(list_common_vars.index("sigGen_Freq")) + hex_float
        )
    ...
    def Write_sigGen_Offset(self, float_value):
        ...
        hex_float = float_to_hex(float_value)
        send_packet = (
            COM_WRITE_VAR + WRITE_FLOAT_SIZE_PAYLOAD + index_to_hex(list_common_vars.index("sigGen_Offset")) + hex_float
        )
        return self._transfer(send_packet, 5)
    ...
    def Write_sigGen_Aux(self, float_value):
        ...
        hex_float = float_to_hex(float_value)
        send_packet = (
            COM_WRITE_VAR + WRITE_FLOAT_SIZE_PAYLOAD + index_to_hex(list_common_vars.index("sigGen_Aux")) + hex_float
        )
        return self._transfer(send_packet, 5)
    ...
    def Write_dp_ID(self, double_value):
        ...
        send_packet = (
            COM_WRITE_VAR + WRITE_DOUBLE_SIZE_PAYLOAD + index_to_hex(list_common_vars.index("dp_ID")) + hex_double
        )
        return self._transfer(send_packet, 5)
    ...
    def Write_dp_Class(self, double_value):
        ...
        hex_double = double_to_hex(double_value)
        send_packet = (
            COM_WRITE_VAR + WRITE_DOUBLE_SIZE_PAYLOAD + index_to_hex(list_common_vars.index("dp_Class")) + hex_double
        )
        return self._transfer(send_packet, 5)
    ...
    def Write_dp_Coeffs(self, list_float):
        ...
        payload_size = size_to_hex(1 + 4 * DP_MODULE_MAX_COEFF)  # Payload: ID + 16floats
        send_packet = COM_WRITE_VAR + payload_size + index_to_hex(list_common_vars.index("dp_Coeffs")) + str_float_list
        return self._transfer(send_packet, 5)
    ...
    def send_wfmref_curve(self, block_idx: int, data) -> bytes:
        ...
        curve_hex = "".join(val)
        send_packet = (
            COM_SEND_WFM_REF + payload_size + index_to_hex(list_curv.index("wfmRef_Curve")) + block_hex + curve_hex
        )
        return self._transfer(send_packet, 5)
    ...
    def recv_wfmref_curve(self, block_idx: int) -> list:
        ...
        send_packet = COM_REQUEST_CURVE + payload_size + index_to_hex(list_curv.index("wfmRef_Curve")) + block_hex
    ...
    def recv_samples_buffer(self) -> list:
        ...
        payload_size = size_to_hex(1 + 2)  # Payload: ID+Block_index
        send_packet = COM_REQUEST_CURVE + payload_size + index_to_hex(list_curv.index("samplesBuffer")) + block_hex
        # Address+Command+Size+ID+Block_idx+data+checksum
    ...
    def send_full_wfmref_curve(self, block_idx: int, data) -> bytes:
        ...
        curve_hex = "".join(val)
        send_packet = (
            COM_SEND_WFM_REF + payload_size + index_to_hex(list_curv.index("fullwfmRef_Curve")) + block_hex + curve_hex
        )
        return self._transfer(send_packet, 5)
    ...
    def recv_full_wfmref_curve(self, block_idx: int) -> list:
        ...
        block_hex = size_to_hex(block_idx)
        payload_size = size_to_hex(1 + 2)  # Payload: ID+Block_index
        send_packet = COM_REQUEST_CURVE + payload_size + index_to_hex(list_curv.index("fullwfmRef_Curve")) + block_hex
        recv_msg = self._transfer(send_packet, 1 + 1 + 2 + 1 + 2 + 16384 + 1)
        val = []
        for k in range(7, len(recv_msg) - 1, 4):
            val.append(struct.unpack("f", recv_msg[k : k + 4]))
    ...
    def recv_samples_buffer_blocks(self, block_idx: int) -> list:
        ...
        payload_size = size_to_hex(1 + 2)  # Payload: ID+Block_index
        send_packet = (
            COM_REQUEST_CURVE + payload_size + index_to_hex(list_curv.index("samplesBuffer_blocks")) + block_hex
        )
        # t0 = time.time()
        # Address+Command+Size+ID+Block_idx+data+checksum
    ...
    def read_curve_block(self, curve_id: int, block_id: int) -> list:
        ...
        send_packet = COM_REQUEST_CURVE + payload_size + index_to_hex(curve_id) + block_hex
        # t0 = time.time()
        self._reset_input_buffer()
        # Address+Command+Size+ID+Block_idx+data+checksum
        recv_msg = self._transfer(send_packet, 1 + 1 + 2 + 1 + 2 + size_curve_block[curve_id] + 1)
        # print(time.time()-t0)
        # print(recv_msg)
        val = []
        for k in range(7, len(recv_msg) - 1, 4):
    ...
    def write_curve_block(self, curve_id: int, block_id: int, data) -> bytes:
        ...
        payload_size = size_to_hex(len(val) * 4 + 3)
        curve_hex = "".join(val)
        send_packet = COM_SEND_WFM_REF + payload_size + index_to_hex(curve_id) + block_hex + curve_hex
        return self._transfer(send_packet, 5)
    ...
    def read_vars_common(self, all=False):
        ...
        resp = {
            ...
            "reference": str(round(self.read_bsmp_variable(2, "float"), 3)) + setpoint_unit,
        }
    ...
    def _include_interlocks(self, vars: dict):
        ...
        if soft_itlks != 0:
            vars["soft_interlocks"] = self.decode_interlocks(soft_itlks, list_fbp_soft_interlocks)

        hard_itlks = self.read_bsmp_variable(32, "uint32_t")
        if hard_itlks != 0:
            vars["hard_interlocks"] = self.decode_interlocks(hard_itlks, list_fbp_hard_interlocks)
        ...
        for index in range(32):
            if reg_interlocks & (1 << index):
                if index < len(list_interlocks):
                    self._interlock_name_assigned(active_interlocks, index, list_interlocks)
                else:
    ...
    def read_vars_fbp(self, n: int = 1, dt: float = 0.5):
        ...
        for _ in range(n):
            ...
            vars = {
                "load_current": str(round(self.read_bsmp_variable(33, "float"), 3)) + " A",
                "load_voltage": str(round(self.read_bsmp_variable(34, "float"), 3)) + " V",
                "load_resistance": str(
                    abs(
                        round(
                            self.read_bsmp_variable(34, "float") / self.read_bsmp_variable(33, "float"),
                            3,
                        )
                    )
                )
                ...
                + " W",
                "dc_link_voltage": str(round(self.read_bsmp_variable(35, "float"), 3)) + " V",
                "heatsink_temp": str(round(self.read_bsmp_variable(36, "float"), 3)) + " °C",
        ...
        try:
            for i in range(n):
                ...
                vars = {
                    ...
                    "dc-link_voltage": str(round(self.read_bsmp_variable(34, "float"), 3)) + " V",
                    "ps1_voltage": str(round(self.read_bsmp_variable(35, "float"), 3)) + " V",
                    "ps2_voltage": str(round(self.read_bsmp_variable(36, "float"), 3)) + " V",
                    "ps3_voltage": str(round(self.read_bsmp_variable(37, "float"), 3)) + " V",
                    "dig_pot_tap": self.read_bsmp_variable(38, "uint8_t"),
                }

                hard_itlks = self.read_bsmp_variable(32, "uint32_t")
                if hard_itlks:
                    vars["hard_interlocks"] = self.decode_interlocks(hard_itlks, list_fbp_dclink_hard_interlocks)
    ...
    def read_vars_fac_acdc(self, n=1, dt: float = 0.5, iib: bool = True):
        ...
        for i in range(n):
            ...
            vars = {
                "measurement_number": i + 1,
                "capbank_voltage": str(round(self.read_bsmp_variable(33, "float"), 3)) + " V",
                "rectifier_current": str(round(self.read_bsmp_variable(34, "float"), 3)) + " A",
            ...
            if iib_is_itlks:
                vars["is"]["decoded_interlocks"] = self.decode_interlocks(iib_is_itlks, list_fac_acdc_iib_is_interlocks)

            iib_is_alarms = self.read_bsmp_variable(46, "uint32_t")
            if iib_is_alarms:
                vars["is"]["decoded_alarms"] = self.decode_interlocks(iib_is_alarms, list_fac_acdc_iib_is_alarms)

            iib_cmd_itlks = self.read_bsmp_variable(57, "uint32_t")
            if iib_cmd_itlks:
                vars["cmd"]["decoded_interlocks"] = self.decode_interlocks(
                    iib_cmd_itlks, list_fac_acdc_iib_cmd_interlocks
                )

            iib_cmd_alarms = self.read_bsmp_variable(58, "uint32_t")
            if iib_cmd_alarms:
                vars["cmd"]["decoded_alarms"] = self.decode_interlocks(iib_cmd_alarms, list_fac_acdc_iib_cmd_alarms)

            if iib:
                iib = {
                    "is": {
                        "input_current": str(round(self.read_bsmp_variable(36, "float"), 3)) + " A",
                        "input_voltage": str(round(self.read_bsmp_variable(37, "float"), 3)) + " V",
                        "igbt_temp": str(round(self.read_bsmp_variable(38, "float"), 3)) + " °C",
                        "driver_voltage": str(round(self.read_bsmp_variable(39, "float"), 3)) + " V",
                        "driver_current": str(round(self.read_bsmp_variable(40, "float"), 3)) + " A",
                        "inductor_temp": str(round(self.read_bsmp_variable(41, "float"), 3)) + " °C",
                        "heatsink_temp": str(round(self.read_bsmp_variable(42, "float"), 3)) + " °C",
                        "board_temp": str(round(self.read_bsmp_variable(43, "float"), 3)) + " °C",
                        "board rh": str(round(self.read_bsmp_variable(44, "float"), 3)) + " %",
                        "interlocks": iib_is_itlks,
                        "alarms": iib_is_alarms,
                    },
                    "cmd": {
                        "load_voltage": str(round(self.read_bsmp_variable(47, "float"), 3)) + " V",
                        "capbank_voltage": str(round(self.read_bsmp_variable(48, "float"), 3)) + " V",
                        "rectifier_inductor_temp": str(round(self.read_bsmp_variable(49, "float"), 3)) + " °C",
                        "rectifier_heatsink_temp": str(round(self.read_bsmp_variable(50, "float"), 3)) + " °C",
                        "external_boards_voltage": str(round(self.read_bsmp_variable(51, "float"), 3)) + " V",
                        "auxiliary_board_current": str(round(self.read_bsmp_variable(52, "float"), 3)) + " A",
                        "idb_board_current": str(round(self.read_bsmp_variable(53, "float"), 3)) + " A",
                        "ground_leakage_current": str(round(self.read_bsmp_variable(54, "float"), 3)) + " A",
                        "board_temp": str(round(self.read_bsmp_variable(55, "float"), 3)) + " °C",
                        "board_rh": str(round(self.read_bsmp_variable(56, "float"), 3)) + " %",
                        "interlocks": iib_cmd_itlks,
                        "alarms": iib_cmd_alarms,
                    },
                }
        ...
        try:
            for i in range(n):
                ...
                if iib:
                    ...
                    print(
                        "IIB Cmd External Boards Voltage: " + str(round(self.read_bsmp_variable(51, "float"), 3)) + " V"
                    )
                    print(
                        "IIB Cmd Auxiliary Board Current: " + str(round(self.read_bsmp_variable(52, "float"), 3)) + " A"
                    )
                    print("IIB Cmd IDB Board Current: " + str(round(self.read_bsmp_variable(53, "float"), 3)) + " A")
                    print(
                        "IIB Cmd Ground Leakage Current: " + str(round(self.read_bsmp_variable(54, "float"), 3)) + " A"
                    )
        ...
        try:
            for i in range(n):
                ...
                if iib:
                    print(
                        "\nIIB CapBank Voltage: "
                        + str(round(self.read_bsmp_variable(40 + iib_offset, "float"), 3))
                        + " V"
                    )
                    print(
                        "IIB Input Current: " + str(round(self.read_bsmp_variable(41 + iib_offset, "float"), 3)) + " A"
                    )
                    print(
                        "IIB Output Current: " + str(round(self.read_bsmp_variable(42 + iib_offset, "float"), 3)) + " A"
                    )
                    print(
                        "IIB IGBT Leg 1 Temp: "
                        + str(round(self.read_bsmp_variable(43 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB IGBT Leg 2 Temp: "
                        + str(round(self.read_bsmp_variable(44 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB Inductor Temp: " + str(round(self.read_bsmp_variable(45 + iib_offset, "float"), 3)) + " °C"
                    )
                    print(
                        "IIB Heat-Sink Temp: "
                        + str(round(self.read_bsmp_variable(46 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB Driver Voltage: " + str(round(self.read_bsmp_variable(47 + iib_offset, "float"), 3)) + " V"
                    )
                    print(
                        "IIB Driver Current 1: "
                        + str(round(self.read_bsmp_variable(48 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB Driver Current 2: "
                        + str(round(self.read_bsmp_variable(49 + iib_offset, "float"), 3))
                        + " A"
                    )
        ...
        try:
            for i in range(n):
                ...
                if not iload == 0:
                    print(
                        "\nLoad Resistance: "
                        + str(abs(round(self.read_bsmp_variable(43, "float") / iload, 3)))
                        + " Ohm"
                    )
                else:
                ...
                print(
                    ...
                    + str(
                        abs(
                            round(
                                self.read_bsmp_variable(43, "float") * self.read_bsmp_variable(33, "float"),
                                3,
                            )
                        )
                    )
            ...
            for i in range(n):
                ...
                if not iib == 0:
                    print(
                        ...
                        + str(iib)
                        + " Input Voltage: "
                        + str(round(self.read_bsmp_variable(58 + iib_offset, "float"), 3))
                        + " V"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Output Voltage: "
                        + str(round(self.read_bsmp_variable(59 + iib_offset, "float"), 3))
                        + " V"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " IGBT 1 Current: "
                        + str(round(self.read_bsmp_variable(60 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " IGBT 2 Current: "
                        + str(round(self.read_bsmp_variable(61 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " IGBT 1 Temp: "
                        + str(round(self.read_bsmp_variable(62 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " IGBT 2 Temp: "
                        + str(round(self.read_bsmp_variable(63 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Driver Voltage: "
                        + str(round(self.read_bsmp_variable(64 + iib_offset, "float"), 3))
                        + " V"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Driver Current 1: "
                        + str(round(self.read_bsmp_variable(65 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Driver Current 2: "
                        + str(round(self.read_bsmp_variable(66 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Inductor Temp: "
                        + str(round(self.read_bsmp_variable(67 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Heat-Sink Temp: "
                        + str(round(self.read_bsmp_variable(68 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        ...
                        + str(round(self.read_bsmp_variable(69 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Board Temp: "
                        + str(round(self.read_bsmp_variable(70 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Board RH: "
                        + str(round(self.read_bsmp_variable(71 + iib_offset, "float"), 3))
                        + " %"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Interlocks: "
                        + str(round(self.read_bsmp_variable(72 + iib_offset, "uint32_t"), 3))
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Alarms: "
                        + str(round(self.read_bsmp_variable(73 + iib_offset, "uint32_t"), 3))
                    )
            ...
            for i in range(n):
                ...
                if not iib == 0:
                    ...
                    print(
                        ...
                        + str(iib)
                        + " Driver Current 2: "
                        + str(round(self.read_bsmp_variable(72 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Inductor Temp: "
                        + str(round(self.read_bsmp_variable(73 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Heat-Sink Temp: "
                        + str(round(self.read_bsmp_variable(74 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Ground Leakage Current: "
                        + str(round(self.read_bsmp_variable(75 + iib_offset, "float"), 3))
                        + " A"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Board Temp: "
                        + str(round(self.read_bsmp_variable(76 + iib_offset, "float"), 3))
                        + " °C"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Board RH: "
                        + str(round(self.read_bsmp_variable(77 + iib_offset, "float"), 3))
                        + " %"
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Interlocks: "
                        + str(round(self.read_bsmp_variable(78 + iib_offset, "uint32_t"), 3))
                    )
                    print(
                        "IIB "
                        + str(iib)
                        + " Alarms: "
                        + str(round(self.read_bsmp_variable(79 + iib_offset, "uint32_t"), 3))
                    )
